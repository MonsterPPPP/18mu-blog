...
export default function RandomWalk({ startId }: { startId?: string }) {
  ...
  if (state === 'loading') return <div className="map-loading" role="status">正在准备漫步路线</div>;
  if (state === 'empty') return <div className="empty-state" role="status"><Footprints size={26} aria-hidden="true" /><h2>漫步路线暂时不可用</h2><p>语义快照尚未取得；你仍可以返回博客阅读全部文章。</p><a className="text-link" href="/blog">返回博客</a></div>;
